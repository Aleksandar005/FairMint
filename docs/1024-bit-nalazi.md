...
| plain gcd (no Bezout) | 414k gas | composition needs gcd only; xgcd dragged 4 muls/iter |
| specialized squaring | **456k / squaring** | doubling case: h=0, s=t, gcd(s,u)=1, so the congruence solve shrinks to half width |

Net: **16.3M -> ~0.46M per group operation, roughly 35x.**

A later session added the missing structural piece, NUCOMP for general
composition (van der Poorten, Math. Comp. 72, 2003, Algorithm 3, transcribed
from the paper and validated against the classic compose on 1600+ Python cases
and 12 Solidity differential vectors). It partially reduces while composing, so
intermediates stay ~|D|^(1/2) instead of growing to |D| and needing a ~150-step
reduce. A single composition dropped from 5.6M to about 1M, and a full measured
verification (80 squarings + 74 compositions, in one call) went from 489M to
**228M gas** — a 2.15x cut on top of everything before it, and roughly 10x from
the first working 1024-bit version. It still exceeds one L1 block, so continuable
verification across a handful of transactions (or an L2) remains the deployment
shape, but the gap is now small. The library was
division-bound; most steps attacked the division inner loop, and the last one is
algebraic: squaring (the bulk of a verification) collapses to a half-width xgcd. Every
step was checked against the Python vectors, the property tests and the fuzz suite. On
full-size forms a squaring now costs 456k and a generic composition 668k (small-
coefficient forms like the generator compose cheaper, 414k, which is why the single
headline number depends on the operand).
...
Using full-size-form costs (the honest ones, since verification runs on arbitrary
forms, not the small generator):

| challenge | operations | verify gas | vs 36M block |
|---|---|---|---|
| 80-bit (demo) | 80 sq + 40 mul | ~63M | ~1.75x |
| 128-bit (production) | 128 sq + 64 mul | ~101M | ~2.8x |
